...
\begin{figure}
  ...
  \includegraphics[scale=0.3]{control.pdf}\\
  \caption{控制节点}\label{control}
\end{figure}
...
\begin{figure}
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[scale=0.3]{node1.pdf}\\
  \caption{计算节点\textbf{1}}\label{node1}
\end{figure}
\begin{figure}
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[scale=0.3]{node2.pdf}\\
  \caption{计算节点\textbf{2}}\label{node2}
\end{figure}
...
\section{提交的文件结构}\label{sec:sec4}
\begin{itemize}
    \item doc文件夹下的report.pdf是技术报告
    \item src文件夹下的sybg文件夹下的是latex_src中是latex源码，code文件夹下式实验代码。
    \item code文件夹下的20185227018node.py是计算节点代码，运行该实验时，应将该该代码部署到不同的服务器上面去，同时将HOST变量修改为本机的IP地址，并启动该程序
    \item code文件夹下的20185227018control.py是管理节点代码
    \item 指定的计算节点如何将最后的汇总结果发送给管理节点
